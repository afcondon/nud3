...
filterTransformToList nodePredicate dataTransform tree = foldl fn Nil tree
  where
  fn acc d =
    if nodePredicate d then Cons (dataTransform d) acc
    else acc
...
filterTransformToListRecursive treePredicate dataTransform tree = catMaybes $ fromFoldable $ go tree
  where
  go :: Tree a -> Tree (Maybe b)
  go subtree@(Node a cs) =
    if treePredicate subtree then Node (Just (dataTransform a)) (go <$> cs)
